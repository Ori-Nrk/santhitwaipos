...
export async function GET(request: NextRequest) {
  try {
    ...
    const result = await query(
      `SELECT * FROM store_settings WHERE user_id = $1 LIMIT 1`,
      [userId]
    )

    // 🔥 First-time user → return default
    if (result.rows.length === 0) {
      return NextResponse.json({
        storeInfo: {
          name: 'My Store',
...
export async function POST(request: NextRequest) {
  try {
    ...
    const existing = await query(
      `SELECT id FROM store_settings WHERE user_id = $1 LIMIT 1`,
      [userId]
    )
    ...
    if (existing.rows.length > 0) {
      ...
      result = await query(
        `INSERT INTO store_settings
         (user_id, store_name, address, phone_number, tax_rate,
          show_logo, show_address, show_phone, footer_message,
          low_stock_alerts, daily_summary, new_sale_alert, theme_color)
    ...
    return NextResponse.json(
      { error: 'Unauthorized or failed to save settings' },
      { status: 401 }
    )
  }
}